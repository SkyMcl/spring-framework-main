...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// Use defaults if no transaction definition given.使用默认的事务定义
		...
		// 判断如果事务定义内的超时时间小于事务默认时间则抛出异常
		if (def.getTimeout() < TransactionDefinition.TIMEOUT_DEFAULT) {
			throw new InvalidTimeoutException("Invalid transaction timeout", def.getTimeout());
		}

		// No existing transaction found -> check propagation behavior to find out how to proceed.
		// 判断事务定义内的传播特性是否是mandatory 是的话抛出异常
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		else
			// 判断事务定义内的传播特性 是否是required 或者 required_New 或者 nested
	...
	private TransactionStatus handleExistingTransaction(
			...
			throws TransactionException {
		// 判断传播特性是否是 never  永不执行事务
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}
		// 判断传播特性是否是 not_support  不支持事务，有事务就挂起
		...
		// 判断传播特性是否是 Required_New  不管外层有没有事务都新建一个事务
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_REQUIRES_NEW) {
			...
			try {
				...
				// 出现运行时异常 或 error的时候 触发 after操作
				resumeAfterBeginException(transaction, suspendedResources, beginEx);
				// 抛出异常
				throw beginEx;
			}
		}
		// 判断传播特性是否是 Nested  外层有事务则用同一个事务，但是会创建保存点，没有事务则会新建一个事务
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			// 判断事务是否是嵌套的  不是嵌套的 就抛出异常
		...
		if (isValidateExistingTransaction()) {
			...
			if (definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT) {
				...
				// 当隔离级别为空 或者当前隔离级别 不等于事务内的隔离级别时抛出异常
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				// 设置ThreadLocalMap内的当前的事务名称为null
				TransactionSynchronizationManager.setCurrentTransactionName(null);
				// 获取当前事务的是否只读属性
				boolean readOnly = TransactionSynchronizationManager.isCurrentTransactionReadOnly();
				// 设置ThreadLocalMap内的当前的事务的只读属性为false
				TransactionSynchronizationManager.setCurrentTransactionReadOnly(false);
				// 获取当前事务的隔离级别 读未提交（幻读、脏读、不可重复读） 读已提交（不可重复读、幻读） 序列化（）  可重复读（幻读）
	...
	protected final void resume(@Nullable Object transaction, @Nullable SuspendedResourcesHolder resourcesHolder)
		...
		if (resourcesHolder != null) {
			...
			// 唤醒后 执行重新设置threadLocalMap内的事务属性
